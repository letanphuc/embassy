...
Vagrant.configure("2") do |config|
  ...
  config.vm.hostname = "rust-stm32-dev"
  ...
  config.vm.provision "shell", inline: <<-SHELL
    apt-get update
    apt-get install -y curl usbutils stlink-tools libusb-1.0-0-dev libudev-dev build-essential libudev-dev pkg-config git
    sudo -u vagrant -i bash -c 'curl --proto '=https' --tlsv1.2 -sSf https://sh.rustup.rs | sh -s -- -y'
    sudo -u vagrant -i bash -c 'cargo install probe-run'
